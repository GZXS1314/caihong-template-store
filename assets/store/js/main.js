...
function getPoint() {
	...
	if(isinvitegift == 1 && invitegift_money != 0){
		...
		$('#invitegift').val(invite_gift +"元");
		$('#alert_invitegift').html('<strong>商品返利规则说明：</strong><br/>生成专属商品返利链接后即可开始发送给好友，别人访问你的专属商品返利链接并购买该返利商品金额一次性达到<font color="red">【'+invitegift_money+'】元</font>，你就可以获得商品返利<font color="red">【'+invite_gift+'】元</font>！');
	}else{
...
$(document).ready(function(){
	...
	$("#submit_invitegift_link").click(function(){
		var tid=$("#tid").val();
		if(tid==0){layer.alert('请选择商品！');return false;}
		layer.msg('正在生成商品返利链接...', {icon: 16, time: 9999999})
		$.ajax({
			type: "POST",
			...
			success: function (data) {
				...
				var clipboard;
				var confirmobj = layer.confirm(data.content, {
				  title:'生成商品返利链接成功',shadeClose:true,btn: ['复制','关闭'], success: function(){
					clipboard = new Clipboard('.layui-layer-btn0',{text: function() {return data.content;}});
					clipboard.on('success', function (e) {
						alert('复制成功！');
					});
